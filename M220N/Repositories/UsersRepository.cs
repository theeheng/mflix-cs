    ...
    public class UsersRepository
    {
        ...
        */
        ...
        public async Task<User> GetUserAsync(string email, CancellationToken cancellationToken = default)
        {
            ...
            //
            var userFilter = Builders<User>.Filter.Eq(m => m.Email, email);
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var user = new User() { Email = email, Name = name, HashedPassword = PasswordHashOMatic.Hash(password) };
        ...
        public async Task<UserResponse> LoginUserAsync(User user, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                // new one by passing the IsUpsert update option.
                  ...
                  Builders<Session>.Filter.Eq(x => x.UserId, storedUser.Email),
                  Builders<Session>.Update.Set(x => x.UserId, user.Email).Set(x => x.Jwt, user.AuthToken),
            ...
            try
            {
                ...
                */
                ...
                updateResult = await _usersCollection.UpdateOneAsync(
                   Builders<User>.Filter.Eq( m=> m.Email, email),
