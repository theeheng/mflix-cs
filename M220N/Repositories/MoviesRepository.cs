...
namespace M220N.Repositories
{
    public class MoviesRepository
    {
        ...
        {
            var projectionFilter = new BsonDocument("score", new BsonDocument("$meta", "textScore"));
            var sort = new BsonDocument("score", new BsonDocument("$meta", "textScore"));

            return await _moviesCollection
                .Find(Builders<Movie>.Filter.Text(string.Join(",", keywords)))
                .Project<MovieByTextProjection>(projectionFilter)
                .Sort(sort)
                .Limit(limit)
                .Skip(page * limit)
                .ToListAsync(cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
           ...
           */
            ...
            var skipStage = new BsonDocument("$skip", page * DefaultMoviesPerPage);
